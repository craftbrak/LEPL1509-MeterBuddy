    ...
    override fun Content() {
        ...
        Scaffold(
            ...
            Column(
                ...
                Log.d("LineChartsScreen", "meters: ${mainPageScreenModel.state.value.meters}")
                if (mainPageScreenModel.state.value.meters.isNotEmpty()) {
                    LazyColumn(Modifier.fillMaxSize()) {
                        items(MeterType.entries.toList()) { type ->
                            val meters =
                                mainPageScreenModel.state.value.meters.filter { it.meterType == type }
                            var readings_production = mutableListOf<MeterReading>()
                            var readings_consumption = mutableListOf<MeterReading>()
                            val unitOfUser =
                                meters.lastOrNull()?.meterUnit ?: MeterUnit.KILO_WATT_HOUR
                                ...
                                meters.forEach { meter ->
                                    ...
                                    if (meter.additiveMeter) {
                                        ...
                                        if (readings_consumption.size > idxFirstReadings_consumption) {
                                            val currentEnergyConsumed =
                                                readings_consumption[idxFirstReadings_consumption].value
                                        ...
                                        if (readings_production.size > idxFirstReadings_production) {
                                            val currentEnergyProduced =
                                                readings_production[idxFirstReadings_production].value
                            ...
                            if (readings_consumption.size >= 2 || readings_production.size >= 2) {
                                ...
                                if (type == MeterType.ELECTRICITY) {
                                    title = "Electricity"
                                } else if (type == MeterType.GAS) {
                                    title = "Gas"
                                } else if (type == MeterType.HOT_WATER) {
                                    title = "Hot water"
                                } else if (type == MeterType.WATER) {
                                    title = "Water"
                                } else if (type == MeterType.CAR) {
                                    title = "Car"
                                } else {
                                    title = "Other"
                                }
